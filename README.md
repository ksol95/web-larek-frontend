...
## 5. Базовые классы
  ...
  - `setText(HTMLElement, unknown)` устанавливает контнет переданный вторым параметром метода, переданному вторым параметром, элементу. Предварительно контент приводится к типу `string`;

  - `setDisabled(HTMLElement, boolean)` добавляет/убирает аттрибут disbled переданому элементу;
  ...
  Из первого объекта копирует свойства в экзэмпляр класса. Скопированые свойства не обязательные.
  Второй аргумент конструктора - объект событий `IEvents` (описан далее). 
  
  #### Метод

  - `emitChanges(string, object)`- Метод создаёт событие измения данных в моделе.
  ...
  ```
  Создаёт массив типа "ключ-значения" где ключ - название события, а значение - функция событие. Данный **массив событий** хранится в свойствпе класса `_events`.
  ...
  - `EmitterEvent` - Интерфейс, описывающтй массив `map` "название события" - "функция";
  - `IEvents`- Интерфейс, определюящий класс `EventEmitter`.

  #### Методы

  - `on(EventName,callback): void`- Записывает в массив событий обработчик - callback по ключу указаному в EventName;
  - `off(EventName,callback)`-  Удаляет из массива событий обработчик с указанного события;
  - `emit(eventName, data?)`-  Инициирует событие, с возомжностью передать обработчику события параметры при необходимости;
  - `onAll(callback: (event: EmitterEvent) => void)` Устанавливает один обработчик на все события;
  - `offAll()` Удаляет все обработчики событий переопределя свойство `_events` пустым массивом событий и их обработчиков.
...
## Модели данных
  ...
  ### CartModel
  Компонент описан интерфейсом **ICartModel** который расширяет интерфейс **IOrderForm** с данными ведённами пользователем. Данный класс представляет из себя модель данных для работы с корзиной.
