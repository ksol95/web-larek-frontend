...
const productInCatalog_Template = ensureElement<HTMLTemplateElement>(
	selectors.productIncatalog
);
const productInPreview_Template = ensureElement<HTMLTemplateElement>(
	selectors.preview
);
const productInCart_Template = ensureElement<HTMLTemplateElement>(
	selectors.productInCart
);
//Шаблон корзины
const cart_Template = cloneTemplate(
	ensureElement<HTMLTemplateElement>(selectors.cart)
);
//Шаблон заказа
const orderForm_Template = cloneTemplate<HTMLFormElement>(
	ensureElement<HTMLTemplateElement>(selectors.order)
);
const contactsForm_Template = cloneTemplate<HTMLFormElement>(
	ensureElement<HTMLTemplateElement>(selectors.contacts)
);
//Шаблон модального окна
const modalContainer = ensureElement<HTMLElement>(selectors.modal);
//Шаблон сообщения об успешной отправки заказа
const success_Template = cloneTemplate(
	ensureElement<HTMLTemplateElement>(selectors.success)
);
...
event.on(eventList.MODEL_CHANGE, () => {
	...
	const catalog = appData.getProducts().map((product) => {
		const productCard_inCatalog = new ProductCard(
			cloneTemplate(productInCatalog_Template),
			{
...
event.on(eventList.PRODUCT_PREVIEW, (product: ProductView) => {
	const ProductCard_Preview = new ProductCard(
		cloneTemplate(productInPreview_Template),
		{
			onClick: () => {
				...
				event.emit(
					appData.cart.includes(product.id)
						? eventList.PRODUCT_REMOVE
						: eventList.PRODUCT_ADD,
					product
				);
...
event.on(eventList.CART_OPEN, () => {
	...
	const productsList = appData.getCart().map((productData, index) => {
		const productCard_inCart = new ProductCard(
			cloneTemplate(productInCart_Template),
			{
				onClick: () => {
...
event.on(
	eventList.ORDER_PAYMENT_TYPE,
	(payment: PaymentType) => (orderFormView.inputs.payment = payment.type)
);

// Изменилось одно из полей в форме заказа
event.on(
	eventList.ORDER_INPUTS_CHANGE,
...
event.on(
	eventList.CONTACTS_INPUTS_CHANGE,
